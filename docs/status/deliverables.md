...
| P0 PostgreSQL-only catalogue patch | VERIFYING — local checks pass, phase open | Local source boot plus PostgreSQL API contract passed 29/29; isolated invalid-loopback DB simulation confirmed 503/no fallback. Staging/production topology check, release artifact, rollback rehearsal, and production acceptance remain open |
| P0 release source and route | BLOCKED | No Git remote/upstream; broad mixed worktree; documented deploy pull path differs from the server repo path in the existing handoff. Owner must select the destination/path and approve the exact candidate |
...
| V1 production release | BLOCKED | No release destination; P0, P2–P5, production TLS/security, and rollback acceptance remain open |
...
- `node node_modules/vite/bin/vite.js build` — PASS. The final build should be rerun after any
  further application-source edit.
- `git diff --check` — recorded in the dated session after the documentation edits.

The workstation's global npm/npx launchers are broken; local Node binaries and the package scripts
were invoked directly. The local checks do not equal production acceptance. No full security audit,
controlled outage test, Cloudflare change, commit, push, or deployment was performed. See
