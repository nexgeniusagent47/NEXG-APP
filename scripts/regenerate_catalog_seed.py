...
those four things. It is bounded: it emits the same artefacts the original does
(seed SQL + JSON bundle) so nothing downstream has to change.
...
def main():
    ...
    # 4. JSON bundle (client hydration).
    bundle = {
        'version': '2.0.0',
        ...
        'merchants': merchants[:JSON_MERCHANT_LIMIT],
        'allMerchantSummaries': [
            {
                'id': m['id'], 'name': m['name'], 'slug': m['slug'],
                'category': m['category'], 'categoryId': m['categoryId'],
                'subcategory': m['subcategory'], 'subcategoryId': m['subcategoryId'],
                'nairobiArea': m['nairobiArea'], 'rating': m['rating'],
                'ratingCount': m['ratingCount'], 'deliveryTime': m['deliveryTime'],
                'deliveryFee': m['deliveryFee'], 'priceLevel': m['priceLevel'],
                'heroImage': m['heroImage'], 'itemCount': len(m['items']),
            }
            for m in merchants
        ],
    }
