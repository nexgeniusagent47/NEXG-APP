        ...
        >
          ...
          <div className="relative h-44 sm:h-52 bg-cover bg-center flex-shrink-0" style={{ backgroundImage: `url(${treatment.image})` }}>
            ...
            <div className="absolute bottom-4 left-6 right-6 z-10">
              ...
              <p className="text-xs sm:text-sm text-gray-300 line-clamp-1">{spaName}</p>
            </div>
          </div>
          ...
          {step < 4 && (
            <div className="flex items-center justify-between px-6 py-2.5 bg-[#1a1d20] border-b border-white/10 text-xs text-gray-400">
              <span className={step === 1 ? 'text-[#E5B65F] font-bold flex items-center gap-1' : 'text-gray-400'}>
                1. Duration & Time
              </span>
              <ChevronRight size={14} />
              <span className={step === 2 ? 'text-[#E5B65F] font-bold flex items-center gap-1' : 'text-gray-400'}>
                2. Oils & Add-ons
              </span>
              <ChevronRight size={14} />
              <span className={step === 3 ? 'text-[#E5B65F] font-bold flex items-center gap-1' : 'text-gray-400'}>
                3. Villa Details
              </span>
            </div>
          )}
          ...
          <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-grow">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2.5">
                    {treatment.durations.map((d) => (
                      ...
                      >
                        <div className="text-xs text-gray-400 font-medium">{d.duration} Minutes</div>
                        <div className="text-base font-bold text-[#E5B65F] mt-0.5">${d.price}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    >
                      <MapPin size={20} className="text-[#E5B65F] shrink-0 mt-0.5" />
                      <div>
                    ...
                    >
                      <Compass size={20} className="text-[#E5B65F] shrink-0 mt-0.5" />
                      <div>
                      ...
                      >
                        ...
                        <div
                          className={`w-4 h-4 rounded-full border flex items-center justify-center ${
                            selectedOil === oil ? 'border-[#E5B65F] bg-[#E5B65F]' : 'border-gray-500'
                          }`}
                        >
                          ...
                          >
                            <div>
                              <div className="text-xs sm:text-sm font-bold text-white flex items-center gap-1.5">
                                ...
                                <span className="text-xs text-[#E5B65F]">+${addon.price}</span>
                              </div>
                              <p className="text-[11px] text-gray-400 mt-0.5">{addon.description}</p>
                            </div>
                            <div
                              className={`w-5 h-5 rounded-md border flex items-center justify-center ${
                                isAdded ? 'bg-[#E5B65F] border-[#E5B65F] text-black' : 'border-gray-500'
                              }`}
                            >
                              {isAdded && <CheckCircle2 size={14} className="text-black" />}
                            </div>
                          </div>
                        );
                      })}
            ...
            {step === 3 && (
              <div className="space-y-5">
                <div className="p-4 rounded-2xl bg-[#E5B65F]/10 border border-[#E5B65F]/20 text-xs text-gray-200">
                  <div className="font-bold text-[#E5B65F] mb-1 flex items-center gap-1.5 text-sm">
                    <UserCheck size={16} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/15 rounded-xl px-4 py-2.5 text-sm text-white focus:border-[#E5B65F] outline-none"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/5 border border-white/15 rounded-xl px-4 py-2.5 text-sm text-white focus:border-[#E5B65F] outline-none resize-none"
                  />
                </div>
                ...
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-2 text-xs">
                  ...
                  <div className="border-t border-white/10 pt-2 flex justify-between font-bold text-sm text-white">
                    ...
                    <span className="text-[#E5B65F]">${totalPrice}</span>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                <div className="p-4 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-xs text-gray-400">Slot Scheduled</div>
                ...
                {confirmedBooking.therapist && (
                  <div className="p-4 rounded-2xl bg-[#1d2023] border border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <div className="text-xs text-gray-400">Assigned Master Therapist</div>
                        <div className="font-bold text-sm text-white">{confirmedBooking.therapist.name}</div>
                        <div className="text-[11px] text-[#E5B65F] flex items-center gap-1">
          ...
          {step < 4 && (
            <div className="p-4 sm:p-5 bg-[#171a1d] border-t border-white/10 flex items-center justify-between">
              <div>
                <div className="text-[11px] text-gray-400">Total Experience Fee</div>
                <div className="text-lg sm:text-xl font-bold text-[#E5B65F]">${totalPrice}</div>
              </div>
